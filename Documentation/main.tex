...
The first and most important method is to work on in the image and video processing operations. These techniques are crucial for the image recognition technologies because they read the video stream as a sequence on images which are processed before the detection and tracking operations. The reading of the stream is in charge of the OpenCV \cite{opencv} library by running a while loop that reads the image sequence from the video capture object.
