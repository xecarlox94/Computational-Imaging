...
\For { $frame \in video$ }
    ...
    \If { $performDetection$ }
        ...
        \Else
            \State $ball = labelBall(frame)$
            \State $ballTracker = Tracker(ball)$
        \EndIf
...
\Procedure{trainModel}{$data, params$}
...
\Procedure{dec\_list}{$data\_list, decode\_fun, unit\_length$}
    if len(data\_list) == 0:
        return data\_list
...
\Procedure{decode\_camera\_data}{$enc\_data$}
    ...
    \State $def get\_pitch\_vec(v, i):$
        \State $index = i + 1$
        \State $z = 0$

        \State $if index in [5, 7, 34, 36]: z = 2.4$

        \State $return (v[0], v[1], z)$


    \State $pitch\_vectors\_decoded = [$
        \State $get\_pitch\_vec($
            \State $pitch\_vectors\_decoded[i],$
            \State $i$
        \State $) for i in range(len($
            \State $pitch\_vectors\_decoded$
        \State $))$
    \State $]$

    \State $return ($
        \State $origin\_decoded,$
        \State $frames\_vectors\_decoded,$
        \State $pitch\_vectors\_decoded$
    \State $)$
